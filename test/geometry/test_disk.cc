...
int main()
{
    ...
    if (abs(Vector(0.0, 0.0, 1.0)*Vector(disk.normal()) - 1.0) > 1e-7)
        throw std::runtime_error("Unexpected disk normal");

    // check point contained predicates
    if (disk.contains(Point(1.0+1e-6, 0.0, 0.0)))
        throw std::runtime_error("False positive on contained point");

    if (!disk.contains(Point(1.0, 0.0, 0.0)))
        throw std::runtime_error("False negative on contained point");
    ...
    if (!e.contains(Point(1.0, 0.0, 0.0)))
        throw std::runtime_error("False negative on contained point on ellipse");

    if (e.contains(Point(1.0-1e-6, 0.0, 0.0)))
        throw std::runtime_error("False positive on contained point on ellipse");
