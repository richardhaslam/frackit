...
int main()
{
    ...
    const Disk disk(Point(0.0, 0.0, 0.0), e1, e2, 1.0, 1.0);
    const auto diskShape = OCCUtilities::getShape(disk);

    const Disk disk2(Point(0.0, 0.0, 10.0), e1, e2, 1.0, 1.0);
    ...
    for (std::size_t i = 1; i <= numTests; ++i)
    {
        ...
        testDisks.emplace_back(Point(0.0, 0.0, 0.0), majAxis, e2, 0.5, 0.5);
